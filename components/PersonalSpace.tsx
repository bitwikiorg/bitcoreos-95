...
export function PersonalSpace() {
  ...
  if (loading) return <div className="personal-loading">Loading identity…</div>;
      ...
      <div className="personal-space anonymous-space">
        <header className="personal-header win-panel raised">
          <div className="personal-identity">
            ...
            <div><div className="mini-title">IDENTITY // PUBLIC SESSION</div><h1>Anonymous</h1><p>public access · no personal projection</p></div>
          </div>
        ...
        <section className="personal-body win-panel raised anonymous-body">
          <div className="anonymous-primary">
            <div className="mini-title">IDENTITY + MINE</div>
            <h2>Public mode is ready.</h2>
            <p>Ask, Research, and Explore work without an account. Sign in to add your identity, private conversations, saved state, contributions, and other Mine projections to those same modes.</p>
            <div className="anonymous-actions">
              <a className="spectral-button inline-action" href="/api/auth/login">Sign in</a>
          ...
          <div className="anonymous-capabilities">
            <div><b>Public</b><span>Knowledge, discussions, public CORE runs, feeds, agents, source reading, search, and research planning.</span></div>
            <div><b>Signed in</b><span>Mine: contribution trail, private authority, saved and watched state, notifications, and identity-linked projections.</span></div>
          </div>
        </section>
      </div>
    );
  ...
  if (!data) return <div className="personal-gate win-panel raised">Unable to load identity.</div>;
    ...
    <div className="personal-space">
      <header className="personal-header win-panel raised">
        <div className="personal-identity">
          ...
          <div><div className="mini-title">IDENTITY // MINE</div><h1>{data.profile.name || `@${data.username}`}</h1><p>@{data.username} · trust level {data.profile.trustLevel}</p></div>
        </div>
      ...
      <nav className="personal-tabs" aria-label="Mine projections">
        <button data-active={tab === 'trail'} onClick={() => setTab('trail')}>Activity</button>
        <button data-active={tab === 'wiki'} onClick={() => setTab('wiki')}>Knowledge</button>
        ...
        {tab === 'trail' && (
          <div className="trail-view">
            <div className="contribution-mix" aria-label="Recent contribution mix">
              <div className="mix-copy"><b>Contribution mix</b><span>Real retrieved activity.</span></div>
        ...
        {tab === 'wiki' && (
          <div className="wiki-personal-view">
            <div className="namespace-card">
              <div className="mini-title">USER NAMESPACE MATCH</div>
              <h2>{data.wiki.namespace.title}</h2>
              <p>{data.wiki.namespace.exists ? 'A public MediaWiki User: page exists under the same username. It remains a username match until the identity bridge is verified.' : 'No public User: page exists under the same username. A verified identity bridge can later make this a durable user-owned knowledge surface.'}</p>
              <a href={data.wiki.namespace.url} target="_blank" rel="noreferrer">Open namespace ↗</a>
            </div>
            <div className="wiki-contrib-list">
              <div className="personal-section-head"><div><b>Knowledge contributions</b><span>MediaWiki contribution history for the same username.</span></div></div>
        ...
        {tab === 'inbox' && (
          <div className="private-workspace">
            <div className="delegated-card">
              <div className="mini-title">DELEGATED PRIVATE STATE</div>
              {data.delegated.connected ? (
                <div className="delegated-status"><div><h2>Private state connected</h2><p>Saved state and notifications are projected through your scoped account authority. Conversations remain in Ask.</p></div><button onClick={disconnect}>Disconnect</button></div>
              ) : (
                <><h2>Identity connected</h2><p>Add scoped private authority for saved, tracked, watched, notification, and conversation projections.</p><a className="spectral-button inline-action" href="/api/auth/user-key/start?return=/my">Connect private state</a></>
              )}
            </div>
            ...
            {data.delegated.connected && (
              <div className="private-columns">
                ...
                <section>
                  <div className="personal-section-head"><div><b>Inbox</b><span>Recent account notifications.</span></div><small>{data.delegated.notifications.length}</small></div>
...
function ActivityRow({ item }: { item: any }) {
  return (
    <a className="activity-row" href={item.url} target="_blank" rel="noreferrer">
      ...
      <div><strong>{item.title}</strong><p>{item.excerpt || 'Open source activity.'}</p></div>
